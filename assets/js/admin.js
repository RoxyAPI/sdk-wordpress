...
( function () {
	...
	function getStrings() {
		...
		const injected = window.RoxyAPIAdmin && window.RoxyAPIAdmin.strings;
		if ( ! injected ) {
			return defaults;
		}
		const out = {};
	...
	function runTestConnection( button, strings ) {
		const result = button.parentNode
			? button.parentNode.querySelector(
					'.roxyapi-test-connection-result'
			  )
			: null;
		const banner = document.getElementById( 'roxyapi-test-banner' );
		...
		wp.apiFetch( { path: '/roxyapi/v1/test-key', method: 'GET' } )
			.then( function ( data ) {
				...
				const msg = humaniseError( data && data.message, strings );
				setResult( result, msg, 'is-error' );
				showBanner( banner, msg, 'is-error' );
			} )
			.catch( function ( err ) {
				const msg = humaniseError( err && err.message, strings );
				setResult( result, msg, 'is-error' );
				showBanner( banner, msg, 'is-error' );
			} );
	}
	...
	function scrollToShortcodes() {
		const target = document.getElementById( 'roxyapi-shortcode-section' );
	...
	function wireCopyButtons() {
		const strings = getStrings();
		document
			.querySelectorAll( '[data-roxyapi-copy]' )
			.forEach( function ( button ) {
				button.addEventListener( 'click', function ( event ) {
					event.preventDefault();
					const code =
						button.getAttribute( 'data-roxyapi-copy' ) || '';
					if ( ! code ) {
						return;
					}
					copyText( code )
