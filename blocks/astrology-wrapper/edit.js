...
const SIGNS = [
	{
		label: __( 'No default (each child block decides)', 'roxyapi' ),
		value: '',
	},
			...
			<InspectorControls>
				<PanelBody title={ __( 'Default values', 'roxyapi' ) }>
					<SelectControl
						...
						options={ SIGNS }
						onChange={ ( defaultSign ) =>
							setAttributes( { defaultSign } )
						}
					/>
					<TextControl
						...
						onChange={ ( defaultBirthDate ) =>
							setAttributes( { defaultBirthDate } )
						}
					/>
				</PanelBody>
			</InspectorControls>
