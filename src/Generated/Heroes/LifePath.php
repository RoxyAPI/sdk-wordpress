...
class LifePath {
	...
	public static function fetch_for_form( array $atts ) {
		...
		if ( $atts['birth_date'] === '' ) {
			return new \WP_Error( 'roxyapi_missing_attrs', sprintf( /* translators: %s is the canonical example shortcode. */ __( "The birth_date attribute is required. Example: %s", 'roxyapi' ), "[roxy_life_path birth_date=\"1990-05-15\"]" ) );
		}

		$birth_date_parts = \RoxyAPI\Support\HeroTransforms::split_iso_date_into_year_month_day( $atts['birth_date'] );

		if ( $birth_date_parts === null ) {
			return new \WP_Error( 'roxyapi_invalid_format', sprintf( /* translators: %s is the canonical example shortcode. */ __( "The birth_date attribute must be in YYYY-MM-DD format. Example: %s", 'roxyapi' ), "[roxy_life_path birth_date=\"1990-05-15\"]" ) );
		}
