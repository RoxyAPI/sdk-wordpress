<?php
/**
 * Auto-generated shortcode: [roxy_get_current_moon_phase]
 *
 * Get current moon phase - Lunar phase calculator with zodiac sign
 *
 * DO NOT EDIT. Generated by bin/generate.mjs. Edit the generator instead.
 *
 * @package RoxyAPI
 */

namespace RoxyAPI\Generated\Shortcodes;

use RoxyAPI\Blocks\Renderer;

class GetCurrentMoonPhase {

	public const TAG = 'roxy_get_current_moon_phase';

	public static function register(): void {
		if ( shortcode_exists( self::TAG ) ) {
			return;
		}
		add_shortcode( self::TAG, array( self::class, 'render' ) );
	}

	public static function render( $atts, $content = '', $tag = '' ): string {
		$atts = shortcode_atts(
			array(
			'lang' => '',
			'date' => '',
			'time' => '',
			'timezone' => '',
			),
			is_array( $atts ) ? $atts : array(),
			(string) $tag
		);

		wp_enqueue_style( 'roxyapi-frontend' );

		$data = \RoxyAPI\Generated\Client::getCurrentMoonPhase( $atts['lang'], $atts['date'], $atts['time'], $atts['timezone'] );

		if ( is_wp_error( $data ) ) {
			return \RoxyAPI\Support\Templates::error( $data->get_error_message() );
		}

		return Renderer::render_generic( 'getCurrentMoonPhase', is_array( $data ) ? $data : array() );
	}
}
