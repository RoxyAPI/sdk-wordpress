...
class PrivacyPolicy {
	...
	public static function register_content(): void {
		...
		$content = '<p>'
			. esc_html__( 'When a visitor submits a RoxyAPI reading form on this site (for example a synastry or compatibility chart), the form data is sent to the third-party RoxyAPI service at https://roxyapi.com to compute the reading. The site owner controls when these forms appear by placing the matching shortcode or block on a page.', 'roxyapi' )
			. '</p><p>'
			...
			. '</p><p>'
			. esc_html__( 'How long RoxyAPI stores the data: see the RoxyAPI privacy policy at https://roxyapi.com/policy/privacy.', 'roxyapi' )
			. '</p><p>'
			. esc_html__( 'No visitor data is sent to RoxyAPI when a page renders a static-attribute shortcode (the data is supplied by the site owner). Form-mode shortcodes are the only case where visitor input crosses to the third party.', 'roxyapi' )
			. '</p>';

		wp_add_privacy_policy_content( 'RoxyAPI', $content );
	}
}
