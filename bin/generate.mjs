...
/**
 * Whether the site supplies a parameter on its own, so a sample must leave it
 * out. The reading language follows the site language and the Branding
 * setting, and an explicit attribute wins over both: a pasted `lang="en"`
 * would pin one placement to English on a translated site.
 * @param field Parameter or body field carrying `name`.
 */
function isSiteResolved( field ) {
	...
	const entries = operations.map( ( op ) => {
		...
		if ( ! hasRequiredObjectBody( op ) ) {
			...
			for ( const p of params ) {
				if ( isCurrentDefaulting( p ) || isSiteResolved( p ) ) {
					continue;
				}
...
function emitHeroPhp( tagSuffix, cfg ) {
	...
	// compatibility, forecast) are a clean alias for the form-mode shortcode.
	// Their input nests below the flat attributes a shortcode can carry, so the
	// hero renders the Form class on every invocation. No DEFAULTS beyond the
	// tag, no fetch_for_form, no companion <Hero>Form class: the long-tail one
	// is reused.
